...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
    ...
    for (int i=0; i<count; i++) {
        if (i<count-1) {
            ...
            GCDAsyncWritePacket *packet = [[GCDAsyncWritePacket alloc] initWithData:data1 timeout:timeout tag:tag];
            ...
            GCDAsyncWritePacket *packet = [[GCDAsyncWritePacket alloc] initWithData:data2 timeout:timeout tag:tag];
